...
class FlatComplexBankStatementGenerator:
    def __init__(self):
        ...
        self.complex_merchants = [
            "WOOLWORTHS METRO", "COLES EXPRESS", "IGA XPRESS", "BUNNINGS WAREHOUSE",
            "HARVEY NORMAN FLAGSHIP", "JB HI-FI HOME", "OFFICEWORKS BUSINESS",
            "CHEMIST WAREHOUSE DISCOUNT", "PRICELINE PHARMACY", "TERRY WHITE",
            "MCDONALD'S FAMILY RESTAURANT", "SUBWAY FRESH FIT", "DOMINO'S PIZZA",
            "RED ROOSTER", "HUNGRY JACK'S", "PIZZA HUT DELIVERY",
            "WESTFIELD SHOPPING CENTRE", "CENTRO SHOPPING", "EASTLAND SHOPPING",
            "PACIFIC FAIR", "GARDEN CITY", "MARION SHOPPING CENTRE"
        ]
    ...
    def generate_complex_transaction_description(self, transaction_category: str) -> str:
        ...
        return pattern.format(
            ...
            name=random.choice(["JOHN SMITH", "SARAH JONES", "MIKE CHEN", "LISA BROWN"]),
            ...
            provider=random.choice(["Origin Energy", "AGL", "Energy Australia", "Red Energy"]),
            ...
            category=random.choice(["Office Supplies", "IT Equipment", "Professional Dev"]),
            ...
            website=random.choice(["amazon.com.au", "ebay.com.au", "kogan.com", "catch.com.au"]),
            country=random.choice(["USA", "UK", "NZ", "SG"])
        )
    
    def generate_unique_dates(self, num_transactions: int, days_span: int = 30) -> List[datetime]:
    ...
    def determine_transaction_type(self, transaction_category: str) -> str:
        ...
        credit_categories = {
            "salary_payments", "government_payments", "investment_transactions"
        }
        return "credit" if transaction_category in credit_categories else "debit"
    
    def generate_flat_complex_statement(self, 
                                      account_holder: str = "CHRISTOPHER PAUL WHITE",
                                      num_transactions: int = 35) -> Dict:
        ...
        for date in chronological_dates:
            ...
            weights = [0.15, 0.12, 0.10, 0.15, 0.12, 0.15, 0.08, 0.08, 0.03, 0.02]  # Sum = 1.0
            ...
            transaction = {
                ...
                "deposit": deposit, 
                "balance": current_balance.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP),
    ...
    def generate_flat_png(self, statement_data: Dict, output_path: str) -> None:
        ...
        # Bank header
        draw.text((50, y_pos), statement_data["bank_name"], fill=header_color, font=title_font)
        y_pos += 35
        ...
        for transaction in statement_data["transactions"]:
            ...
            if transaction["withdrawal"]:
                draw.text((550, y_pos), f"${transaction['withdrawal']:.2f}", fill=text_color, font=small_font)
            
            # Deposit column  
            if transaction["deposit"]:
                draw.text((650, y_pos), f"${transaction['deposit']:.2f}", fill=text_color, font=small_font)
            
            # Balance column
            draw.text((750, y_pos), f"${transaction['balance']:.2f}", fill=text_color, font=small_font)
        ...
        # Summary information
        draw.text((50, y_pos), f"Total Transactions: {statement_data['transaction_count']}", 
                 fill=text_color, font=normal_font)
        y_pos += 16
        draw.text((50, y_pos), f"Closing Balance: ${statement_data['closing_balance']:.2f}", 
                 fill=header_color, font=normal_font)
...
def generate_flat_complex_bank_statement(output_filename: str = "commbank_flat_complex.png",
                                       num_transactions: int = 35) -> Dict:
