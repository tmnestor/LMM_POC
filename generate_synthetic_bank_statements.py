...
class AustralianBankStatementGenerator:
    ...
    def generate_transaction(self, transaction_type: str, date: datetime) -> Dict:
        ...
        description = pattern.format(
            ref=self.reference_generators.get(
                "payroll", lambda: str(random.randint(10000, 99999))
            )(),
    ...
    def calculate_running_balance(
        self, transactions: List[Dict], opening_balance: Decimal
    ) -> List[Dict]:
        ...
        for transaction in sorted_transactions:
            ...
            transaction["balance"] = balance.quantize(
                Decimal("0.01"), rounding=ROUND_HALF_UP
            )
    ...
    def generate_statement_data(
        ...
        for _ in range(remaining_transactions):
            ...
            weights = [
                ...
                0.10,
                0.10,
            ]  # 6 types remaining after salary
        ...
        transactions_with_balance = self.calculate_running_balance(
            transactions, opening_balance
        )

        # Calculate totals
        total_credits = sum(
            t["amount"] for t in transactions_with_balance if t["type"] == "credit"
        )
        total_debits = sum(
            t["amount"] for t in transactions_with_balance if t["type"] == "debit"
        )
        closing_balance = (
            transactions_with_balance[-1]["balance"]
        ...
        display_transactions = sorted(
            transactions_with_balance, key=lambda x: x["date"], reverse=True
        )
    ...
    def map_to_extraction_fields(
        self, statement_data: Dict, image_filename: str
    ) -> Dict[str, str]:
        ...
        for csv_field, data_key in self.field_schema_mapping.items():
            if data_key is None:
                ...
                if (
                    csv_field == "TRANSACTION_DATES"
                    and data_key == "transaction_dates_chronological"
                ):
                    # Transaction dates in chronological order (oldest first)
                    chronological_transactions = self.get_chronological_transactions(
                        transactions
                    )
                    dates = [
                        t["date"].strftime("%d/%m/%Y")
                        for t in chronological_transactions
                    ]
                    result[csv_field] = " | ".join(dates) if dates else "NOT_FOUND"

                elif (
                    csv_field == "TRANSACTION_AMOUNTS_PAID"
                    and data_key == "debit_amounts_only"
                ):
                    ...
                    result[csv_field] = (
                        " | ".join(debit_amounts) if debit_amounts else "NOT_FOUND"
                    )

                elif (
                    csv_field == "TRANSACTION_AMOUNTS_RECEIVED"
                    and data_key == "credit_amounts_only"
                ):
                    ...
                    result[csv_field] = (
                        " | ".join(credit_amounts) if credit_amounts else "NOT_FOUND"
                    )

                elif (
                    csv_field == "TRANSACTION_BALANCES"
                    and data_key == "running_balances"
                ):
                    # Running balances with CR/DR suffixes (chronological order)
                    balances = self.get_running_balances(transactions)
                    result[csv_field] = (
                        " | ".join(balances) if balances else "NOT_FOUND"
                    )
    ...
    def generate_ground_truth_csv(
        self, statements_data: List[Dict], csv_path: str, append_mode: bool = False
    ) -> None:
        ...
        with csv_file.open(mode, newline="", encoding="utf-8") as f:
            ...
            for statement_data in statements_data:
                image_filename = statement_data.get(
                    "image_filename",
        ...
        print(
            f"📊 Fields mapped: {len(fieldnames)} columns, {len(statements_data)} rows"
        )
    ...
    def generate_batch(
        ...
        for i in range(count):
            ...
            print(
                f"Generated statement {statement_num}: {statement_data['account_holder'][:20]}... - "
        ...
        if with_ground_truth:
            ...
            self.generate_ground_truth_csv(
                statements, str(csv_path), append_mode=append_csv
            )
...
def generate_all_banks(
    count_per_bank: int = 5, output_base_dir: str = "australian_synthetic_statements"
) -> Dict[str, List[Dict]]:
    ...
    generator.generate_ground_truth_csv(
        combined_statements, str(combined_dir / "ground_truth_all_banks.csv")
    )
...
def generate_single_bank(
    bank: str = "CommBank", count: int = 10, output_dir: str = None
) -> List[Dict]:
    ...
    return generator.generate_batch(
        count=count, bank=bank, output_dir=output_dir, with_ground_truth=True
    )
...
def generate_one_per_bank(output_dir: str = "big4_bank_statements") -> Dict[str, Dict]:
    ...
    summary = {
        ...
        "files_generated": [
            f"{bank.lower()}_statement_001.png" for bank in generator.banks.keys()
        ],
...
def main():
    ...
    avg_balance = sum(
        parse_balance(stmt["closing_balance"]) for stmt in statements
    ) / len(statements)
