...
Two-step API: processor.apply_chat_template() for chat formatting,
processor() for image tokenization.  Thinking mode disabled via
...
# Reuse Qwen3VL config — same shape, same defaults
QWEN35_GENERATION_CONFIG = dict(QWEN3VL_GENERATION_CONFIG)


class DocumentAwareQwen35Processor(BaseDocumentProcessor):
    ...
    def __init__(
        ...
        if self.debug:
            print(f"Qwen3.5 processor initialized for {self.field_count} fields")
