...
- Supports FP8 quantization via FbgemmFp8Config for L40S/H100
...
class DocumentAwareLlama4Processor(BaseDocumentProcessor):
    ...
    def __init__(
        ...
        if self.debug:
            print(f"Llama 4 Scout processor initialized for {self.field_count} fields")
    ...
    def _resilient_generate(
        ...
        assert oom  # Always True here; satisfies mypy reachability
        gc.collect()
        torch.cuda.empty_cache()
        if self.debug:
