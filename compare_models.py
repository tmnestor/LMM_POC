...
class ModelComparator:
    ...
    def create_side_by_side_comparison(
        ...
        """Create side-by-side performance comparison dashboard."""
        ...
        models = ["Llama-3.2-11B", "InternVL3-2B"]
        accuracies = [
            llama_data.get("overall_accuracy", 0) * 100,
            internvl3_data.get("overall_accuracy", 0) * 100
        ]
        ...
        stats_categories = ["Documents\nProcessed", "Perfect\nDocuments"]
        llama_stats = [llama_docs, llama_data.get("perfect_documents", 0)]
        internvl3_stats = [internvl3_docs, internvl3_data.get("perfect_documents", 0)]

        x_pos = np.arange(len(stats_categories))
        width = 0.35

        bars2_1 = ax2.bar(x_pos - width/2, llama_stats, width, label="Llama", 
                         color=VIZ_COLORS["primary"], alpha=0.8)
        ...
        llama_field_accs = llama_data.get("field_accuracies", {})
        internvl3_field_accs = internvl3_data.get("field_accuracies", {})

        # Count fields by quality level
        ...
        llama_counts = count_quality_levels(llama_field_accs)
        internvl3_counts = count_quality_levels(internvl3_field_accs)

        quality_categories = ["Excellent\n(≥90%)", "Good\n(80-90%)", "Poor\n(<80%)"]
        quality_colors = [VIZ_COLORS["success"], VIZ_COLORS["info"], VIZ_COLORS["warning"]]

        x_pos = np.arange(len(quality_categories))
        bars3_1 = ax3.bar(x_pos - width/2, llama_counts, width, label="Llama",
                         color=VIZ_COLORS["primary"], alpha=0.8)
        ...
        performance_metrics = ["Best Document\nAccuracy", "Worst Document\nAccuracy"]
        llama_perf = [
            llama_data.get("best_performance_accuracy", 0) * 100,
            llama_data.get("worst_performance_accuracy", 0) * 100
        ]
        internvl3_perf = [
            internvl3_data.get("best_performance_accuracy", 0) * 100,
            internvl3_data.get("worst_performance_accuracy", 0) * 100
        ]

        x_pos = np.arange(len(performance_metrics))
        bars4_1 = ax4.bar(x_pos - width/2, llama_perf, width, label="Llama",
                         color=VIZ_COLORS["primary"], alpha=0.8)
    ...
    def create_field_accuracy_heatmap(
        ...
        """Create field-by-field accuracy heatmap comparison."""
        print("🎨 Creating field accuracy heatmap...")

        # Extract field accuracies
        llama_accs = llama_data.get("field_accuracies", {})
        internvl3_accs = internvl3_data.get("field_accuracies", {})

        # Create comparison matrix
        all_fields = sorted(set(llama_accs.keys()) | set(internvl3_accs.keys()))
        comparison_data = []

        for field in all_fields:
            llama_acc = llama_accs.get(field, 0) * 100
            internvl3_acc = internvl3_accs.get(field, 0) * 100
            delta = llama_acc - internvl3_acc

            comparison_data.append({
                "Field": field,
                "Llama": llama_acc,
                "InternVL3": internvl3_acc,
                "Delta": delta
            })
        ...
        fig.suptitle("Field-Level Performance Comparison", fontsize=16, fontweight="bold")

        # Left plot: Side-by-side accuracy heatmap
        heatmap_data = comparison_df[["Llama", "InternVL3"]].to_numpy()
        
        im1 = ax1.imshow(heatmap_data, cmap="RdYlGn", aspect="auto", vmin=0, vmax=100)
        ax1.set_title("Accuracy by Field (%)", fontweight="bold")
        ax1.set_xticks([0, 1])
        ax1.set_xticklabels(["Llama", "InternVL3"])
        ax1.set_yticks(range(len(all_fields)))
        ax1.set_yticklabels(all_fields, fontsize=9)
        ...
        # Right plot: Performance delta (Llama - InternVL3)
        delta_data = comparison_df["Delta"].to_numpy().reshape(-1, 1)
        
        im2 = ax2.imshow(delta_data, cmap="RdBu_r", aspect="auto", 
                        vmin=-max(abs(delta_data)), vmax=max(abs(delta_data)))
        ax2.set_title("Performance Delta\n(Llama - InternVL3)", fontweight="bold")
        ax2.set_xticks([0])
        ax2.set_xticklabels(["Δ Accuracy"])
        ax2.set_yticks(range(len(all_fields)))
        ax2.set_yticklabels(all_fields, fontsize=9)

        # Add delta values
        for i in range(len(all_fields)):
            color = "white" if abs(delta_data[i, 0]) > 20 else "black"
            text = ax2.text(0, i, f"{delta_data[i, 0]:+.1f}%",
...
def find_latest_results(output_dir: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    llama_file = max(llama_files, key=lambda x: x.stat().st_mtime) if llama_files else None
    
    # Find latest InternVL3 results  
    internvl3_files = list(output_path.glob("internvl3_evaluation_results_*.json"))
    internvl3_file = max(internvl3_files, key=lambda x: x.stat().st_mtime) if internvl3_files else None
    
    return (str(llama_file) if llama_file else None, 
...
def main():
    ...
    parser.add_argument(
        "--llama", 
        type=str, 
        help="Path to Llama evaluation JSON file"
    )
    parser.add_argument(
        "--internvl3",
        type=str,
        help="Path to InternVL3 evaluation JSON file"
    )
    parser.add_argument(
        "--auto",
        action="store_true",
    ...
    try:
        ...
        dashboard_path = comparator.create_side_by_side_comparison(llama_data, internvl3_data)
        generated_files.append(dashboard_path)

        # 2. Field accuracy heatmap
        heatmap_path = comparator.create_field_accuracy_heatmap(llama_data, internvl3_data)
        generated_files.append(heatmap_path)

        # 3. Executive summary
        summary_path = comparator.generate_comparison_summary(llama_data, internvl3_data)
