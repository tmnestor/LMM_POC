...
def create_multiline_bank_statement(
    ...
    y += 20
    draw.text(
        (30, y), f"Account Holder: {account_holder}", fill="gray", font=font_small
    )
    y += 16
    ...
    for i, txn in enumerate(transactions):
        ...
        if txn["credit"]:
            ...
            draw.text(
                (630 - credit_width, y + 12), txn["credit"], fill="green", font=font
            )
        ...
        draw.text(
            (760 - balance_width, y + 12), txn["balance"], fill=balance_color, font=font
        )
...
def main():
    ...
    # Check min balance
    balances = [
        float(t["balance"].replace("$", "").replace(",", "")) for t in transactions
    ]
