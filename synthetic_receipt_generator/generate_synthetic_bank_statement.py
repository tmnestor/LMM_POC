...
def generate_transactions(
    ...
    for _ in range(num_transactions):
        ...
        if is_debit:
            # Debits: $10 to $2000
            amount = Decimal(str(random.uniform(10, 2000))).quantize(
            ...
            # Credits: $100 to $5000
            amount = Decimal(str(random.uniform(100, 5000))).quantize(
                Decimal("0.01"), rounding=ROUND_HALF_UP
            )
            debit = Decimal("0")
            credit = amount
...
def create_bank_statement_image(
    ...
    for i, txn in enumerate(transactions):
        ...
        if i % 2 == 0:
            draw.rectangle(
                [(40, y - row_height + 5), (width - 40, y + 5)], fill="#f8f8f8"
            )

        draw.text((50, y - row_height + 8), txn["date"], fill="black", font=font)
        draw.text(
            (170, y - row_height + 8), txn["description"], fill="black", font=font
        )
        ...
        if txn["debit"]:
            ...
            draw.text(
                (750 - debit_width, y - row_height + 8),
                txn["debit"],
                fill="red",
                font=font,
            )
        ...
        if txn["credit"]:
            ...
            draw.text(
                (900 - credit_width, y - row_height + 8),
                txn["credit"],
                fill="green",
                font=font,
            )
        ...
        draw.text(
            (1100 - balance_width, y - row_height + 8),
            txn["balance"],
            fill="black",
            font=font,
        )
...
def main():
    ...
    rprint(
        "[yellow]Note: Add ground truth rows to evaluation_data/bank/ground_truth_bank.csv[/yellow]"
    )
