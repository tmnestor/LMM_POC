...
class ATOTaxInvoiceGenerator:
    ...
    def generate_invoice_png(self, invoice_data: Dict, output_path: str) -> None:
        ...
        for item in invoice_data["line_items"]:
            ...
            draw.text(
                (400, y_pos), str(item["quantity"]), fill=text_color, font=small_font
            )
            draw.text(
                (500, y_pos),
            ...
            draw.text(
                (650, y_pos), f"${item['total']:.2f}", fill=text_color, font=small_font
            )
            y_pos += 20
        ...
        # Payment terms
        draw.text(
            (50, y_pos), invoice_data["payment_terms"], fill=text_color, font=small_font
        )
        y_pos += 30
    ...
    def map_to_extraction_fields(
        self, invoice_data: Dict, image_filename: str
    ) -> Dict[str, str]:
    ...
    def generate_ground_truth_csv(
        self, invoices_data: List[Dict], csv_path: str, append_mode: bool = False
    ) -> None:
        ...
        with csv_file.open(mode, newline="", encoding="utf-8") as f:
            ...
            for invoice_data in invoices_data:
                image_filename = invoice_data.get(
                    "image_filename",
                    f"invoice_{invoice_data.get('invoice_number', 'unknown')}.png",
                )
    ...
    def generate_batch(
        self,
        count: int = 10,
        output_dir: str = "synthetic_invoices",
        ...
        for i in range(count):
            ...
            print(
                f"Generated invoice {invoice_num}: {invoice_data['invoice_number']} - {invoice_data['total_amount']} "
        ...
        if with_ground_truth:
            ...
            self.generate_ground_truth_csv(
                invoices, str(csv_path), append_mode=append_csv
            )
        ...
        summary = {
            ...
            "under_1000": sum(
                1 for inv in invoices if not inv["requires_buyer_details"]
            ),
...
def main():
    ...
    invoices = generator.generate_batch(
        count=20, output_dir="ato_synthetic_invoices", with_ground_truth=True
    )

    print("\n📊 Sample Invoice Data:")
    print(f"   Total invoices: {len(invoices)}")
    print(
        f"   Average amount: ${sum(float(inv['total_amount'][1:]) for inv in invoices) / len(invoices):.2f}"
    )
