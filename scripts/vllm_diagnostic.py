...
def check_python(report: Report) -> None:
    ...
    detail = f"found {v.major}.{v.minor}.{v.micro}; need {lo_maj}.{lo_min}–{hi_maj}.{hi_min}"
...
def check_nvidia_smi(report: Report) -> None:
    if not shutil.which("nvidia-smi"):
        report.add("nvidia_smi", "fail", "nvidia-smi not found in PATH — no NVIDIA driver?")
        return
    driver = _run(["nvidia-smi", "--query-gpu=driver_version", "--format=csv,noheader"])
    cuda_runtime = _run(["nvidia-smi", "--query-gpu=cuda_version", "--format=csv,noheader"])
    gpus = _run(
        [
            "nvidia-smi",
    ...
    report.system["cuda_driver_max"] = cuda_runtime.splitlines()[0] if cuda_runtime else None
...
def check_torch(report: Report, expected_gpus: int | None) -> None:
    ...
    report.system["torch_cudnn"] = torch.backends.cudnn.version() if torch.cuda.is_available() else None
    ...
    try:
        ...
        report.add(
            "torch_cxx11_abi",
            "info",
            f"_GLIBCXX_USE_CXX11_ABI = {cxx11} (must match any out-of-tree CUDA kernel wheel)",
        )
    except AttributeError:
        pass
    ...
    if torch.cuda.is_available():
        ...
        report.add(
            ...
            "ok",
            f"torch built with CUDA {torch.version.cuda}, {dev_count} device(s) visible",
        )
...
def check_flash_attention_absent(report: Report) -> None:
    ...
    if backend:
        status = (
            "ok"
            if backend.upper() in {"XFORMERS", "TORCH_SDPA", "FLASHINFER", "TRITON_ATTN_VLLM_V1"}
            else "warn"
        )
        ...
        report.add(
            ...
            "info",
            "VLLM_ATTENTION_BACKEND unset — recommend XFORMERS on A10G when flash-attn is unavailable",
        )
...
def check_hard_deps(report: Report) -> None:
    ...
    for pkg, want in hard.items():
        try:
            ...
            report.add(f"hard_dep:{pkg}", status, f"installed {have}; vLLM pins {pkg}=={want}")
...
def build_report(target: str, expected_gpus: int | None = None) -> Report:
    if target not in VLLM_MATRIX:
        raise SystemExit(f"Unknown vLLM target {target!r}. Known: {sorted(VLLM_MATRIX)}")
...
def print_human(report: Report) -> int:
    ...
    if req.get("hard_deps"):
        print(" Pinned hard deps:     " + ", ".join(f"{k}=={v}" for k, v in req["hard_deps"].items()))
    print()
    print(" --- System ---")
...
def main() -> int:
    ...
    p.add_argument(
        ...
        help=f"target vLLM version (default: {DEFAULT_TARGET}). Known: {sorted(VLLM_MATRIX)}",
    )
    p.add_argument(
        "--expected-gpus",
        type=int,
