...
def _evaluate_scalar(class_name: str, pred_vals: list[str], gt_vals: list[str]) -> ClassResult:
...
def _evaluate_list_position_aware(class_name: str, pred_vals: list[str], gt_vals: list[str]) -> ClassResult:
...
def evaluate_image(predicted: dict, gt: WildReceiptGT) -> ImageResult:
    ...
    for class_name in CLASS_NAMES:
        ...
        if not gt_vals and not pred_vals:
            ...
            result.per_class[class_name] = _evaluate_list_position_aware(class_name, pred_vals, gt_vals)
        else:
            result.per_class[class_name] = _evaluate_scalar(class_name, pred_vals, gt_vals)
...
def save_results_csv(results: list[BenchmarkResult], output_dir: Path) -> Path:
    ...
    with summary_path.open("w", newline="") as f:
        ...
        for r in results:
            ...
            for class_name in CLASS_NAMES:
                cr = r.class_results[class_name]
                row.extend([round(cr.precision, 4), round(cr.recall, 4), round(cr.f1, 4)])
            writer.writerow(row)
    ...
    with detail_path.open("w", newline="") as f:
        ...
        for r in results:
            for img in r.image_results:
                ...
                for class_name in CLASS_NAMES:
                    cr = img.per_class.get(class_name, ClassResult(class_name=class_name))
                    row.extend([cr.true_positives, cr.false_positives, cr.false_negatives])
                writer.writerow(row)
...
def main(
    ...
    console.print(f"[bold]WildReceipt Benchmark[/bold] | Models: {', '.join(model)} | Data: {data_dir}")
