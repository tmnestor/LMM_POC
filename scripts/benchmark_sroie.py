...
def _run_inference_internvl3(model, tokenizer, image: Image.Image, prompt: str, max_tokens: int) -> str:
...
def _run_inference_llama4scout(model, processor, image: Image.Image, prompt: str, max_tokens: int) -> str:
...
def _run_inference_granite4(model, processor, image: Image.Image, prompt: str, max_tokens: int) -> str:
...
def _run_inference_nemotron(model, processor, image: Image.Image, prompt: str, max_tokens: int) -> str:
...
def _run_inference_generic(model, processor, image: Image.Image, prompt: str, max_tokens: int) -> str:
...
def _run_inference_vllm(
    ...
    outputs = model.chat(messages=messages, sampling_params=sampling, use_tqdm=False, **chat_kwargs)
...
def run_inference(
    ...
    if model_type in ("internvl3", "internvl3-14b", "internvl3-38b"):
        return _run_inference_internvl3(model, tokenizer_or_processor, image, prompt, max_tokens)
    if model_type == "llama4scout":
        return _run_inference_llama4scout(model, tokenizer_or_processor, image, prompt, max_tokens)
    if model_type in (
        "llama4scout-w4a16",
        "internvl3-vllm",
        "internvl3-14b-vllm",
        ...
        return _run_inference_vllm(model, tokenizer_or_processor, image, prompt, max_tokens, model_type)
    if model_type == "granite4":
        return _run_inference_granite4(model, tokenizer_or_processor, image, prompt, max_tokens)
    if model_type == "nemotron":
        return _run_inference_nemotron(model, tokenizer_or_processor, image, prompt, max_tokens)
    # Fallback for qwen3vl and other models using two-step API
    return _run_inference_generic(model, tokenizer_or_processor, image, prompt, max_tokens)
...
def run_benchmark(
        ...
        if run_config_path.exists():
            ...
            if isinstance(default_paths, dict) and model_type in default_paths:
                ...
                if candidate.exists():
                    ...
                    console.print(f"Model path from run_config.yml: {resolved_model_path}")
    ...
    # vLLM engine tuning lives in run_config.yml and is resolved via AppConfig;
    # the loader requires it for vLLM backends (HF backends ignore it).
    ...
    with registration.loader(cfg, app_config=app_cfg) as (model, tokenizer_or_processor):
...
def print_results_table(results: list[SROIEBenchmarkResult]) -> None:
    ...
    summary.add_row(
        "Img/min",
        *[
            f"{r.total_images / r.elapsed_seconds * 60:.1f}" if r.elapsed_seconds > 0 else "N/A"
            for r in results
        ],
    )
...
def save_results_csv(results: list[SROIEBenchmarkResult], output_dir: Path) -> Path:
    ...
    with summary_path.open("w", newline="") as f:
        ...
        for r in results:
            img_per_min = r.total_images / r.elapsed_seconds * 60 if r.elapsed_seconds > 0 else 0.0
            row: list[Any] = [
                r.model_name,
            ...
            for field in SROIE_FIELDS:
                fr = r.field_results[field]
                row.extend([round(fr.precision, 4), round(fr.recall, 4), round(fr.f1, 4)])
            writer.writerow(row)
...
def main(
    ...
    console.print(f"[bold]SROIE Benchmark[/bold] | Models: {', '.join(model)} | Data: {data_dir}")
