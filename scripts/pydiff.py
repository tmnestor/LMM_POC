...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare two files (no coreutils required)")
    parser.add_argument("file1", type=Path, help="First file")
    parser.add_argument("file2", type=Path, help="Second file")
    parser.add_argument("-s", "--silent", action="store_true", help="Exit code only, no output")
    parser.add_argument("-c", "--context", action="store_true", help="Context diff format")
    parser.add_argument("--html", action="store_true", help="HTML side-by-side diff")
    parser.add_argument("-n", "--lines", type=int, default=3, help="Context lines (default: 3)")
    ...
    if args.html:
        ...
        diff = "".join(difflib.context_diff(a, b, str(args.file1), str(args.file2), n=args.lines))
    else:
        diff = "".join(difflib.unified_diff(a, b, str(args.file1), str(args.file2), n=args.lines))
