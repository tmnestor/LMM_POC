...
class MultiGPUOrchestrator:
    ...
    def __init__(self, config, num_gpus: int, *, shuffle: bool = False, app_config=None) -> None:
        self.config = config
    ...
    def run(
        ...
        for gpu_id, chunk in enumerate(chunks):
            console.print(
                f"  [dim]GPU {gpu_id}: {len(chunk)} images ({chunk[0].name} .. {chunk[-1].name})[/dim]"
            )
        ...
        for gpu_id in range(actual_gpus):
            ...
            model_ctx = load_model(gpu_config, app_config=self.app_config)
        ...
        console.print(f"\n[bold green]Multi-GPU processing complete: {elapsed:.1f}s total[/bold green]")
