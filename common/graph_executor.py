...
class GraphExecutor:
    ...
    def _walk(
        ...
        while current != "done":
            if steps >= self._max_graph_steps:
                msg = f"Graph exceeded {self._max_graph_steps} steps at node '{current}'"
            ...
            result, edge = self._execute_node(current, node_def, state, loaded_images, retries)
    ...
    def _execute_node(
        ...
        match node_def:
            ...
            case {"template": str(template), "edges": dict(), **rest}:
                return self._execute_model_call(key, template, rest, state, images, retries)
    ...
    def _resolve_max_tokens(self, node_def: dict[str, Any]) -> int:
        """Resolve max_tokens for a node definition.

        Priority:
        1. ``token_budget: <name>`` -> resolve via ``budget_resolver``
        2. ``max_tokens: N`` -> use directly (backwards compat)
        3. Fall back to ``default_max_tokens``
        """
        budget_name = node_def.get("token_budget")
        if budget_name is not None and self._budget_resolver is not None:
            return self._budget_resolver(budget_name)
        if "max_tokens" in node_def:
            return node_def["max_tokens"]
    ...
    def _evaluate_router(
        ...
        # Column-based routing (from detect_headers OR classify_document)
        for source in ("detect_headers", "classify_document"):
            if state.has(source):
                mapping = state.get(f"{source}.column_mapping", None)
                if isinstance(mapping, dict):
                    ...
                    if has_amount and "has_amount" in edges:
                        return "has_amount"
                    if (has_debit or mapping.get("credit")) and "has_debit_or_credit" in edges:
    ...
    def _run_validator(
        ...
        match check_name:
            case "amount_gate":
                receipts = state.get("extract_receipts.receipts") if state.has("extract_receipts") else []
                matches = state.get("match_to_statement.matches") if state.has("match_to_statement") else []
            ...
            case "trust_compliance":
                ...
                return run_trust_compliance(state)
            case _:
                return True, {}
                ...
                case {"type": "dedup", "field": str(field_name)}:
                    ...
                    if state.has("validate_amounts"):
                        ...
                        if "matches" in parsed:
                            parsed["matches"] = dedup_by_field(parsed["matches"], field_name)
    ...
    def _build_final_fields(
        ...
        # Trust distribution linking: use validator output
        if state.has("validate_compliance"):
            validated = state.node_results["validate_compliance"].parsed
            for key in (
                "TRUST_ABN",
                "BENEFICIARY_TFN",
                "SHARE_OF_NET_INCOME",
        ...
        if state.has("match_to_statement") or state.has("validate_amounts"):
            ...
            fields["RECEIPT_DATE"] = " | ".join(r.get("DATE", "NOT_FOUND") for r in receipts)
            fields["RECEIPT_DESCRIPTION"] = " | ".join(m.get("RECEIPT_STORE", "NOT_FOUND") for m in matches)
            fields["RECEIPT_TOTAL"] = " | ".join(m.get("RECEIPT_TOTAL", "NOT_FOUND") for m in matches)
