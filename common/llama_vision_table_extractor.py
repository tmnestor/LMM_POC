...
class LlamaVisionTableExtractor:
    ...
    def __init__(self, model_path: str = None, processor=None, model=None):
        ...
        if processor is not None and model is not None:
            ...
            rprint(
                "[green]✅ Using existing model and processor with V100 ResilientGenerator[/green]"
            )

        elif model_path:
            # Load new instances (for standalone usage)
            rprint(
                f"[yellow]Loading new V100-optimized model from: {model_path}[/yellow]"
            )
            ...
            rprint(
                "[green]✅ Standalone V100-optimized model and processor loaded[/green]"
            )
        else:
            raise ValueError(
                "Must provide either model_path OR (processor, model) instances"
            )
    ...
    def extract_table(
        ...
        try:
            ...
            if fragmentation > 0.5:  # V100 threshold
                rprint(
                    f"[yellow]⚠️ Pre-processing fragmentation: {fragmentation:.2f}GB - cleaning up[/yellow]"
                )
            ...
            try:
                ...
                if image.format not in ["PNG", "JPEG", "JPG", None]:
                    rprint(
                        f"[yellow]⚠️ Warning: Unusual image format {image.format}[/yellow]"
                    )
            ...
            rprint(
                f"[yellow]🔍 V100 Resilient extraction from {image_path.name} (max_tokens: {max_new_tokens})...[/yellow]"
            )
            ...
            try:
                ...
                if len(response.strip()) < 10:
                    rprint(
                        "[yellow]⚠️ Warning: Very short model response - possible inference issue[/yellow]"
                    )
            ...
            if fragmentation_after > 1.0:  # Higher threshold for post-processing
                rprint(
                    f"[yellow]⚠️ Post-processing fragmentation: {fragmentation_after:.2f}GB[/yellow]"
                )
                handle_memory_fragmentation(threshold_gb=1.0, aggressive=True)

            # Parse and return structured response
            return self._parse_response(
                response, str(image_path), prompt, temperature, max_new_tokens
            )
        ...
        for line in lines:
            if (
                ...
                if len(cells) >= 4:  # Ensure minimum columns
                    ...
                    if any(
                        value and value != "NOT_FOUND" for value in row_data.values()
                    ):
    ...
    def test_extraction(
        self, image_path: str, prompt: str = None, max_new_tokens: int = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            rprint(
                "[blue]🎯 V100 Features: ResilientGenerator, Memory cleanup, Fragmentation detection[/blue]"
            )
            console.rule("[bold blue]V100 Extraction Test[/bold blue]")

            # V100: Pre-test memory state
            allocated_start, reserved_start, fragmentation_start = (
                detect_memory_fragmentation()
            )
            ...
            result = self.extract_table(
                image_path,
                prompt_template=prompt or MINIMAL_PROMPT,
                max_new_tokens=max_new_tokens,
            )
            ...
            allocated_end, reserved_end, fragmentation_end = (
                detect_memory_fragmentation()
            )
            ...
            analysis_table = Table(
                title="📈 V100 Extraction Analysis", border_style="green"
            )
            ...
            analysis_table.add_row(
                "Transactions Extracted", str(extracted_count), count_status
            )
            analysis_table.add_row(
                "Ground Truth Count", str(expected_count), "📋 From CSV"
            )
            analysis_table.add_row(
                "Processing Time", f"{processing_time:.2f}s", "⏱️ V100 Performance"
            )
            analysis_table.add_row(
                "Max Tokens Used", str(max_new_tokens), "🔧 V100 Configuration"
            )
            analysis_table.add_row(
            ...
            length_status = (
                "✅ Good" if 100 < response_length < 5000 else "⚠️ Check Quality"
            )
            analysis_table.add_row(
                "Response Length", f"{response_length} chars", length_status
            )

            # Hallucination check
            hallucination_patterns = ["IGA", "1102.37"]
            hallucination_detected = any(
                pattern in result["raw_response"]
                and result["raw_response"].count(pattern) > 3
                for pattern in hallucination_patterns
            )
            hallucination_status = (
                "❌ Possible Hallucination" if hallucination_detected else "✅ Clean"
            )
            analysis_table.add_row(
                "Hallucination Check", "Pattern Detection", hallucination_status
            )
            ...
            rprint(
                f"[cyan]🎯 V100 Memory Management: {abs(memory_delta):.2f}GB delta, {fragmentation_change:+.2f}GB fragmentation change[/cyan]"
            )
