...
class ColumnMatcher:
    ...
    def _find_match(self, headers: list[str], headers_lower: list[str], keywords: list[str]) -> str | None:
...
class ResponseParser:
    ...
    def parse_balance_description(
        ...
        for line in lines:
            ...
            if not date_found:
                date_match = re.match(r"^\d+\.\s*\*?\*?\[(\d{1,2}\s+[A-Za-z]{3,9})\]\*?\*?", line)
                if date_match:
                    date_found = date_match.group(1).strip()
            ...
            if not date_found:
                date_match = re.match(r"^\d+\.\s*\*?\*?(\d{1,2}\s+[A-Za-z]{3,9}\s+\d{2})\*?\*?$", line)
                if date_match:
                    date_found = date_match.group(1).strip()
...
class UnifiedBankExtractor:
    ...
    def __init__(
        ...
        self._prompts = {
            ...
            "turn1_debit_credit": catalog.get_prompt("bank", "turn1_debit_credit_extraction"),
    ...
    def _extract_rows(
        self, image: Any, prompt: str, parse_fn: Any
    ) -> tuple[list[dict[str, Any]], dict[str, str]]:
        """Run turn-1 extraction (single full-image pass) and return parsed rows.

        Returns (rows, raw_responses) where raw_responses keys go into
        ExtractionResult for tracing.
        """
    ...
    def extract(
        ...
        cached_column_mapping: ColumnMapping | None = None,
    ) -> ExtractionResult:
        ...
        Args:
            ...
            cached_column_mapping: Optional pre-computed column mapping
                (skips Turn 0 header detection when provided).
        ...
        # Turn 0: Header detection (or use cached mapping)
        if cached_column_mapping is not None:
            self._log("[UBE] Turn 0: Using cached column mapping (skipping header detection)")
            mapping = cached_column_mapping
            headers = [
                v
                for v in [
                    mapping.date,
        ...
        if strategy == ExtractionStrategy.BALANCE_DESCRIPTION:
            result = self._extract_balance_description(image, headers, mapping, turn0_response)
        elif strategy == ExtractionStrategy.AMOUNT_DESCRIPTION:
            result = self._extract_amount_description(image, headers, mapping, turn0_response)
        elif strategy == ExtractionStrategy.DEBIT_CREDIT_DESCRIPTION:
            result = self._extract_debit_credit_description(image, headers, mapping, turn0_response)
        else:
    ...
    def _extract_balance_description(
        ...
        # Turn 1: extract rows (single full-image pass)
        ...
        if self.use_balance_correction:
            ...
            is_chrono, order_reason = BalanceCorrector.is_chronological_order(all_rows, date_col)
            ...
            for i, row in enumerate(corrected_rows):
                ...
                self._log(f"[UBE]     {i}: D={debit_val or 'N/A':10} C={credit_val or 'N/A':10} {desc}")
        else:
    ...
    def _extract_amount_description(
        ...
        self._log(f"[UBE]   Filtered to {len(debit_rows)} debit transactions (negative amounts)")
    ...
    def extract_bank_statement(
        ...
        Args:
            ...
            cached_column_mapping: Optional pre-computed column mapping
                (skips header detection when provided).
        ...
        with _bypass_rich_stdout():
            result: ExtractionResult = self.extract(
                ...
                cached_column_mapping=cached_column_mapping,
            )
        ...
        if self.verbose:
            ...
            _safe_print(f"[UBE]   Transactions extracted: {len(result.transaction_dates)}")
