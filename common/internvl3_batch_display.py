...
{prompt_text[:200]}{"..." if len(prompt_text) > 200 else ""}"""
        ...
        self.console.print(
            Panel(panel_content, title="🔧 Extraction Prompt", border_style="cyan")
        )
    ...
    def display_raw_and_cleaned(self, result: Dict[str, Any]) -> None:
        ...
        rprint("\n")
        self.console.print(
            Panel(table, title="📝 Raw Response vs Cleaned Data", border_style="blue")
        )

    def display_field_comparison(
        self, result: Dict[str, Any], ground_truth: Dict[str, Any], document_type: str
    ) -> None:
        """
        ...
        for field in extracted_data.keys():
            ...
            extracted_display = (
                str(extracted_value)[:30] + "..."
                if len(str(extracted_value)) > 30
                else str(extracted_value)
            )
            gt_display = (
                str(gt_value)[:30] + "..." if len(str(gt_value)) > 30 else str(gt_value)
            )
        ...
        # Summary stats
        rprint(
            f"[bold cyan]📈 Summary: {matches}/{total_fields} fields correct ({accuracy:.1f}% accuracy)[/bold cyan]"
        )
...
def display_field_comparison(
    result: Dict[str, Any], ground_truth: Dict[str, Any], document_type: str
) -> None:
