...
class BatchVisualizer:
    ...
    def create_dashboard(
        ...
        if not df_doctype_stats.empty:
            doc_perf = (
                df_results.groupby("document_type")["overall_accuracy"].mean().sort_values(ascending=True)
            )
            colors = [
                "red" if acc < 60 else "orange" if acc < 80 else "green" for acc in doc_perf.to_numpy()
            ]
    ...
    def create_all_visualizations(
        ...
        self.create_dashboard(df_results, df_doctype_stats, timestamp, dashboard_path, show)
        saved_files["dashboard"] = dashboard_path

        # Create heatmap
        heatmap_path = output_dir / f"field_accuracy_heatmap_{timestamp}.png"
        heatmap_result, field_stats = self.create_field_heatmap(df_results, timestamp, heatmap_path, show)
        if heatmap_result:
