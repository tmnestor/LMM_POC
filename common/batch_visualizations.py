...
class BatchVisualizer:
    ...
    def create_dashboard(
        ...
        sns.histplot(data=df_results, x='overall_accuracy', bins=20, kde=True, 
        ...
        if 'document_type' in df_results.columns and len(df_results['document_type'].unique()) > 1:
            ...
            sns.histplot(data=df_results, x='processing_time', bins=15, kde=True, 
                        ax=ax2, color='coral')
        ...
        if not df_doctype_stats.empty:
            ...
            colors = ['red' if acc < 60 else 'orange' if acc < 80 else 'green' for acc in doc_perf.to_numpy()]
    ...
    def create_field_heatmap(
        ...
        plt.title(f'Field-Level Accuracy Heatmap - {timestamp}', fontsize=14, fontweight='bold')
        plt.xlabel('Document')
        plt.ylabel('Field')
        ...
        }).round(2).sort_values('Average Accuracy (%)', ascending=False)
    ...
    def create_all_visualizations(
        ...
        self.create_dashboard(df_results, df_doctype_stats, timestamp, 
                            dashboard_path, show)
        ...
        if heatmap_result:
            ...
            if field_stats is not None:
                field_stats_path = output_dir.parent / 'csv' / f"batch_{timestamp}_field_stats.csv"
