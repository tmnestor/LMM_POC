...
class ExtractionCleaner:
    ...
    def _clean_list_field(self, field_name: str, value: str) -> str:
        ...
        if any(pattern in field_name.upper() for pattern in ["AMOUNT", "PRICE", "COST", "FEE"]):
        ...
        match (gst_amount, subtotal, total, is_gst_included):
            case (gst, sub, tot, inc) if all(field != "NOT_FOUND" for field in [gst, sub, tot]):
                try:
                    ...
                    if gst_val is not None and subtotal_val is not None and total_val is not None:
                        match inc.lower():
                            case "true":
    ...
    def _validate_transaction_counts(self, data: dict[str, str]) -> dict[str, str]:
        ...
        for field_name, value in [
            ...
            if value != "NOT_FOUND":
                counts[field_name] = len([x.strip() for x in value.split("|") if x.strip()])

        # Check if all counts match
        if counts and len(set(counts.values())) > 1:
            if self.debug:
                print("⚠️  Issue #6: Bank statement transaction count mismatch detected:")
