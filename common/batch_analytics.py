...
class BatchAnalytics:
    ...
    def create_summary_statistics(self, df_results: pd.DataFrame) -> pd.DataFrame:
        ...
        summary_stats = {
            ...
            'Failed Extractions': len(self.batch_results) - len(self.successful_results),
            ...
            'Average Processing Time (s)': np.mean(self.processing_times) if self.processing_times else 0,
            'Total Processing Time (s)': sum(self.processing_times) if self.processing_times else 0,
            'Throughput (images/min)': 60 / np.mean(self.processing_times) if self.processing_times else 0
        }
    ...
    def create_doctype_statistics(self, df_results: pd.DataFrame) -> pd.DataFrame:
        ...
        df_stats = df_results.groupby('document_type').agg({
            'overall_accuracy': ['mean', 'median', 'std', 'min', 'max'],
            'processing_time': ['mean', 'median'],
            'image_name': 'count'
        }).round(2)
        
        df_stats.columns = ['_'.join(col).strip() for col in df_stats.columns.to_numpy()]
        df_stats = df_stats.rename(columns={'image_name_count': 'count'})
        
        return df_stats
    
    def create_field_statistics(self, df_results: pd.DataFrame) -> Optional[pd.DataFrame]:
        """
