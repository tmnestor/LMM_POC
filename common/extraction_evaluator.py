...
class ExtractionEvaluator:
    ...
    def __init__(
        ...
        *,
        ...
        self.evaluation_method = evaluation_method or os.environ.get("EVALUATION_METHOD", "order_aware_f1")
        self._verbose = verbose
        self._model_evaluator = SimpleModelEvaluator()

        # JSONL mode: evaluate exactly the fields in each GT record
        self._jsonl_mode = ground_truth_csv is not None and Path(ground_truth_csv).suffix == ".jsonl"

        # Load ground truth once — fail fast if path given but missing
        self._ground_truth_data: dict[str, dict] = {}
        if ground_truth_csv:
            self._ground_truth_data = load_ground_truth(ground_truth_csv, verbose=verbose)
    ...
    def _evaluate_extraction(
        ...
        if document_type.upper() == "BANK_STATEMENT" and self.enable_math_enhancement:
            ...
            enhanced_result = enhance_bank_statement_extraction(extracted_data, verbose=self._verbose)

            extracted_data = {k: v for k, v in enhanced_result.items() if k != "_mathematical_analysis"}
        ...
        if document_type.upper() == "BANK_STATEMENT" and mathematical_analysis is not None:
        ...
        filtered_ground_truth = _normalize_transaction_order(filtered_ground_truth, document_type)

        if document_type.upper() == "BANK_STATEMENT":
            logger.debug("Evaluating using mathematically corrected values (not raw VLM output)")
        ...
        perfect_matches = sum(1 for score in field_scores.values() if score["f1_score"] == 1.0)
    ...
    def _filter_debit_transactions(self, extracted_data: dict) -> dict:
        ...
        try:
            ...
            if any(field == "" or field == "NOT_FOUND" for field in [descriptions, dates, paid]):
