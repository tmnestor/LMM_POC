...
def _log_gpu_memory(phase: str) -> None:
    ...
    for idx in range(torch.cuda.device_count()):
        ...
        frag = reserved - alloc
        logger.info(
            "[gpu-mem][%s][gpu=%d] alloc=%.2fGiB reserved=%.2fGiB peak=%.2fGiB frag=%.2fGiB",
            phase,
            idx,
            alloc,
            reserved,
...
class DocumentPipeline:
    ...
    def _run_pipeline(
        ...
        for det in detections:
            document_types_found[det.document_type] = document_types_found.get(det.document_type, 0) + 1
        for doc_type, count in sorted(document_types_found.items()):
            logger.info("  %s: %d", doc_type, count)

        # Phase 2: Extraction
        extractions, extraction_batch_sizes = self._extract_all(image_paths, detections, verbose)
        ...
        # Compute batch stats
        avg_detect = (
            sum(detection_batch_sizes) / len(detection_batch_sizes) if detection_batch_sizes else 1.0
        )
        avg_extract = (
            sum(extraction_batch_sizes) / len(extraction_batch_sizes) if extraction_batch_sizes else 1.0
        )
        stats = BatchStats(
    ...
    def _detect_all(
        ...
        if self._batch_size > 1 and self._can_batch:
            ...
            for batch_start in range(0, total_images, self._batch_size):
                ...
                batch_classifications = self._orchestrator.detect_batch(batch_paths, verbose=verbose)
        ...
        logger.info("Detection complete: %.1fs for %d images", detection_time, total_images)
    ...
    def _extract_all(
        ...
        progress_task = progress.add_task("Processing images", total=total_images, current="")
        ...
        if standard_indices and self._batch_size > 1 and self._can_batch:
            ...
            for batch_start_idx in range(0, len(standard_indices), self._batch_size):
                batch_end_idx = min(batch_start_idx + self._batch_size, len(standard_indices))
                batch_orig_indices = standard_indices[batch_start_idx:batch_end_idx]

                batch_paths = [image_paths[i] for i in batch_orig_indices]
                batch_class_infos = [detections[i].classification_info for i in batch_orig_indices]
                ...
                per_image_time = batch_extract_time / len(batch_orig_indices) if batch_orig_indices else 0
    ...
    def _extract_bank(self, detection: DetectionResult, verbose: bool) -> ExtractionOutput:
        ...
        if self._bank_adapter is not None:
            ...
            bank_failed = False
            try:
                schema_fields, metadata = self._bank_adapter.extract_bank_statement(detection.image_path)
