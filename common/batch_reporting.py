...
class BatchReporter:
    ...
    def generate_executive_summary(
        ...
        inference_only = (
            df_results.get("inference_only", pd.Series([False])).any() if len(df_results) > 0 else False
        )
...
"""
        ...
        if not inference_only and len(df_results) > 0:
            ...
            if len(accuracy_results) > 0:
                ...
                for _, row in top_performers.iterrows():
                    report += (
                        f"- {row['image_name']}: {row['overall_accuracy']:.1f}% ({row['document_type']})\n"
                    )
        elif inference_only and len(df_results) > 0:
            # In inference-only mode, show top field extraction results
            top_extractors = df_results.nlargest(min(5, len(df_results)), "fields_extracted")[
        ...
        if not inference_only and len(df_results) > 0:
            ...
            if len(accuracy_results) > 0:
                ...
                below_threshold = poor_performers[poor_performers["overall_accuracy"] < 50]
...
"""
    ...
    def generate_json_report(
        ...
        export_data = {
            ...
            "configuration": {
                ...
                "ground_truth": str(config.ground_truth) if config.ground_truth else None,
    ...
    def save_all_reports(
        ...
        markdown_report = self.generate_executive_summary(df_results, df_doctype_stats, output_dirs["base"])
