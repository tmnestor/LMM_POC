...
def print_accuracy_by_document_type(
    ...
    for doc_type_key, results in sorted(doc_type_results.items()):
        ...
        logger.info("  Median of Medians: %.1f%% - most robust", median_of_medians * 100)
    ...
    if total_docs > 0:
        weighted_median = sum(s["avg_median_f1"] * s["count"] for s in summary.values()) / total_docs
        weighted_mean = sum(s["avg_mean_f1"] * s["count"] for s in summary.values()) / total_docs
