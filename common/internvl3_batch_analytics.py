...
class InternVL3BatchAnalytics:
    ...
    def generate_summary_stats(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Overall accuracy
        overall_accuracy = (
            (total_fields_found / total_fields_processed * 100)
        ...
        return {
            ...
            "throughput_images_per_minute": (total_images / total_time * 60)
            if total_time > 0
            else 0,
    ...
    def display_summary_table(self, summary_stats: Dict[str, Any]) -> None:
        ...
        table.add_row(
            "Successful Extractions", str(summary_stats["successful_extractions"])
        )
        ...
        table.add_row(
            "Avg Processing Time", f"{summary_stats['average_processing_time']:.2f}s"
        )
        table.add_row(
            "Total Processing Time", f"{summary_stats['total_processing_time']:.2f}s"
        )
        table.add_row(
            "Throughput", f"{summary_stats['throughput_images_per_minute']:.1f} img/min"
        )
        table.add_row("", "")  # Separator
        table.add_row(
            "Total Fields Processed", str(summary_stats["total_fields_processed"])
        )
        table.add_row("Total Fields Found", str(summary_stats["total_fields_found"]))
        table.add_row(
            "Overall Field Accuracy", f"{summary_stats['overall_accuracy']:.1f}%"
        )
        ...
        if summary_stats["document_types"]:
            ...
            for doc_type, stats in summary_stats["document_types"].items():
                accuracy = (
                    (stats["fields_found"] / stats["fields_total"] * 100)
                    if stats["fields_total"] > 0
                    else 0
                )
                doc_table.add_row(doc_type, str(stats["count"]), f"{accuracy:.1f}%")
    ...
    def create_simple_dashboard(
        ...
        fig.suptitle(
            "InternVL3 Batch Processing Dashboard", fontsize=16, fontweight="bold"
        )

        # 1. Success Rate Pie Chart
        success_data = [
            summary_stats["successful_extractions"],
            summary_stats["failed_extractions"],
        ]
        ...
        successful_results = [
            r for r in results if "error" not in r and "processing_time" in r
        ]
        ...
        if doc_types:
            ...
            bars = ax3.bar(
                doc_names, doc_counts, color=["#9b59b6", "#f39c12", "#1abc9c"]
            )
        ...
        if doc_types:
            ...
            bars = ax4.bar(
                doc_names, accuracies, color=["#e67e22", "#27ae60", "#8e44ad"]
            )
...
def create_simple_dashboard(
    results: List[Dict[str, Any]], save_path: Optional[Path] = None, show: bool = True
) -> Optional[Path]:
