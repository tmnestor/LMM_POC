...
backend = VllmBackend(engine, model_type_key="internvl3")
generate_fn = backend.generate_for_graph
```

`generate_for_graph()` builds `SamplingParams` from `NodeGenParams`:
- `output_schema=dict()` -> `StructuredOutputsParams(json=schema)`
- `stop=list()` -> stop sequences
- `logprobs=int()` -> per-token logprobs
- Thinking suppression handled internally via `model_type_key`
...
| SamplingParams build | `vllm_backend.py:generate_for_graph()` | `NodeGenParams` fields | vLLM parameter construction |
...
| `models/backends/vllm_backend.py` | `VllmBackend`: vLLM engine backend with `generate()` and `generate_for_graph()` |
