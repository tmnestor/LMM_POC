...
def calculate_field_accuracy(
    ...
    *,
    ...
    if field_types.get(field_name) == "numeric_id":
        ...
        if debug:
            print(f"    📞 PHONE: '{extracted_digits}' vs '{ground_truth_digits}' = {score}")
        return score
        ...
        extracted_items = [item.strip() for item in re.split(r"\s*\|\s*", extracted) if item.strip()]
        ground_truth_items = [item.strip() for item in re.split(r"\s*\|\s*", ground_truth) if item.strip()]
            ...
            for ext_item in extracted_items:
                for gt_item in ground_truth_items:
                    try:
                        ...
                        tolerance = abs(gt_num * _tol) if gt_num != 0 else _tol
        ...
        score = matches / max(len(ground_truth_items), len(extracted_items)) if ground_truth_items else 0.0
        if debug:
            print(
...
def _compare_monetary_values_pair(extracted: str, ground_truth: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if isinstance(ext_val, float) and isinstance(gt_val, float):
            if abs(ext_val - gt_val) < get_field_schema().get_threshold(
    ...
    *,
    ...
    for _idx, result in enumerate(extraction_results):
        ...
        image_overall_accuracy = (
            sum(image_accuracies.values()) / len(image_accuracies) if image_accuracies else 0.0
        )
        ...
        if field_name in fields.transaction_list_fields:
            ...
            if is_monetary_field:
                try:
                    ...
                    # Allow configurable tolerance for rounding (same as calculate_field_accuracy)
                    tolerance = abs(ground_truth_num * _tol) if ground_truth_num != 0 else _tol
