...
class BankStatementCalculator:
    ...
    def analyze_bank_statement(self, extracted_data: dict[str, Any]) -> BankStatementAnalysis:
        """
    ...
    def analyze_bank_statement_legacy(self, extracted_data: dict[str, Any]) -> BankStatementAnalysis:
        """
    ...
    def analyze_bank_statement_pandas(self, extracted_data: dict[str, Any]) -> BankStatementAnalysis:
        """
        ...
        try:
            ...
            descriptions_list = (
                descriptions_parsed[:max_length] if descriptions_parsed else ["Transaction"] * max_length
            )
            ...
            transactions_df = pd.DataFrame(
                {
                    ...
                    "extracted_paid": [
                        abs(x) if x != 0 else np.nan for x in amounts_paid_parsed[:max_length]
                    ],
                    "extracted_received": [
                        abs(x) if x != 0 else np.nan for x in amounts_received_parsed[:max_length]
                    ],
                }
            )
            ...
            def apply_validation(row):
                ...
                final_paid = (
                    corrected_paid if corrected_paid > 0 else (calc_paid if calc_paid > 0 else np.nan)
                )
                final_received = (
                    corrected_received
            ...
            df_calc[["final_paid", "final_received", "validation_reason"]] = df_calc.apply(
                apply_validation, axis=1
            )

            # Debug output - print the COMPLETE corrected DataFrame including descriptions
            if self.verbose:
                logger.debug("MATHEMATICAL CORRECTION DEBUG - COMPLETE CORRECTED DATAFRAME")
                ...
                for _i, row in debug_df.iterrows():
                    ...
                    final_paid_str = (
                        f"${row['final_paid']:.2f}" if pd.notna(row["final_paid"]) else "NOT_FOUND"
                    )
                    final_recv_str = (
                        f"${row['final_received']:.2f}" if pd.notna(row["final_received"]) else "NOT_FOUND"
                    )
                    logger.debug(
                        ...
                        desc_short,
                        row["balance"],
                        row["balance_change"] if pd.notna(row["balance_change"]) else "NaN",
                        row["extracted_paid"] if pd.notna(row["extracted_paid"]) else "NaN",
                        row["extracted_received"] if pd.notna(row["extracted_received"]) else "NaN",
                        final_paid_str,
                        final_recv_str,
                    )
            ...
            analysis = BankStatementAnalysis(
                ...
                else ["Balance validation warning: calculated vs actual balance mismatch"],
            )
    ...
    def _display_pandas_analysis_summary(
        ...
        logger.debug("Net Change: $%.2f", analysis.total_credits - analysis.total_debits)
        ...
        if vlm_corrected_count > 0:
            logger.debug("  %d VLM amounts (classification corrected)", vlm_corrected_count)
        logger.debug("  %d mathematical calculations (VLM gaps filled)", mathematical_count)
    ...
    def _display_earliest_transaction_details(
        ...
        logger.debug("Array Position: %d (position in original extraction data)", array_index)
        ...
        if extracted_paid and len(extracted_paid) > array_index:
            ...
            if paid_amount > 0:
                logger.debug("  TRANSACTION_AMOUNTS_PAID[%d]: $%.2f", array_index, paid_amount)
            else:
                logger.debug(
            ...
            logger.debug("  TRANSACTION_AMOUNTS_PAID[%d]: No data available", array_index)
        ...
        if extracted_received and len(extracted_received) > array_index:
            ...
            logger.debug("  TRANSACTION_AMOUNTS_RECEIVED[%d]: No data available", array_index)
    ...
    def _validate_and_correct_vlm_extraction(
        ...
        if inferred_type:
            if inferred_type == "DEBIT" and vlm_received > 0 and vlm_paid == 0:
                ...
                logger.warning("Description suggests DEBIT but VLM extracted as CREDIT - correcting")
                ...
                logger.warning("Description suggests CREDIT but VLM extracted as DEBIT - correcting")
