...
class FieldSchema:
    ...
    # -- Evaluation thresholds (from YAML evaluation.thresholds) --
    evaluation_thresholds: dict[str, float] = field(default_factory=dict, repr=False)
    ...
    def from_yaml(
        ...
        """
        resolved = config_path or (Path(__file__).parent.parent / "config" / "field_definitions.yaml")
        ...
        if not isinstance(raw, dict):
            raise FieldSchemaError(
                f"Expected top-level mapping in {resolved.absolute()}, got {type(raw).__name__}",
                path=resolved,
            ) from None
        ...
        for doc_type, type_cfg in doc_fields_raw.items():
            if not isinstance(type_cfg, dict) or "fields" not in type_cfg:
                raise FieldSchemaError(
                    f"Document type '{doc_type}' missing 'fields' list in {resolved.absolute()}",
                    path=resolved,
                ) from None
            ...
            doc_type_fields[doc_type] = tuple(f for f in type_cfg["fields"] if f not in _VALIDATION_ONLY)
        ...
        thresholds_raw = eval_cfg.get("thresholds", {})
        eval_thresholds = {k: float(v) for k, v in thresholds_raw.items()}
        ...
        return cls(
            ...
            text_fields=frozenset(f for f in extraction_fields if field_type_map.get(f) == "text"),
    ...
    def get_threshold(self, name: str, default: float) -> float:
        """Return an evaluation threshold by name, or *default* if missing.

        Args:
            name: Threshold name (e.g. "fuzzy_text_match", "monetary_tolerance").
            default: Value returned when *name* is not in the YAML thresholds.
        """
        return self.evaluation_thresholds.get(name, default)
