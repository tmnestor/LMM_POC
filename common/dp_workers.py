"""Data-parallel worker functions.

Each function is a top-level callable invoked in a subprocess by
``vllm_dp.run_dp``. Workers build their own model (single-GPU)
...
# =============================================================================
# Stage 1: Classification (model-agnostic -- works for both HF and vLLM)
# =============================================================================
...
def classify_worker(
    ...
    *,
    ...
    """Worker: load model + processor, classify each image.
...
# =============================================================================
# Stage 2: HF extraction (classified path)
# =============================================================================


def hf_extract_worker(
    gpu_id: int,
    image_paths: list[str],
    *,
    config_path: str | None,
...
# =============================================================================
# Stage 2: HF extraction (graph-based unified/robust path)
# =============================================================================


def hf_unified_worker(
    gpu_id: int,
    image_paths: list[str],
    *,
    config_path: str | None,
    ...
    """Worker: build HF model + GraphExecutor, run graph workflow per image.

    Args:
        gpu_id: GPU rank (for logging).
        image_paths: Absolute paths to images for this chunk.
        config_path: Path to run_config.yml (or None).
        cli_overrides: CLI args dict for AppConfig.load().
        workflow_name: YAML file under prompts/workflows/.
        label: Label for prompt_used prefix (e.g. "robust", "unified").
    ...
    try:
        ...
        def generate_fn(
            image: Any, prompt: str, params: NodeGenParams
        ) -> GenerateResult:
...
# =============================================================================
# Stage 2: vLLM extraction (graph-based unified/robust path)
# =============================================================================


def vllm_extract_worker(
    gpu_id: int,
    image_paths: list[str],
    *,
    config_path: str | None,
