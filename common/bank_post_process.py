...
def run_bank_post_process(
    ...
    if strategy == "extract_balance":
        ...
        sorted_rows = rows if is_chrono else BalanceCorrector.sort_by_date(rows, date_col)
        ...
        # Filter debits
        debit_rows = TransactionFilter.filter_debits(corrected_rows, debit_col=debit_col, desc_col=desc_col)
        ...
        debit_rows = TransactionFilter.filter_debits(rows, debit_col=debit_col, desc_col=desc_col)
...
_CANONICAL_TYPES = frozenset({"RECEIPT", "INVOICE", "BANK_STATEMENT", "TRAVEL", "LOGBOOK"})
...
def run_select_best_type(
    ...
    doc_score = sum(
        ...
        if isinstance(v, str) and v != "NOT_FOUND" and not k.startswith("_") and k.isupper()
    )
    ...
    if bank_score >= 3 and doc_score < 6:
        ...
        if "probe_bank_headers" in state.node_results:
            state.node_results["detect_headers"] = state.node_results.pop("probe_bank_headers")
    else:
